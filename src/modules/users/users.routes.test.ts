...
describe('administrative user routes', () => {
  it('requires authentication and rejects OPERATOR on every endpoint', async () => {
    ...
    expect((await instance.inject('/users')).statusCode).toBe(401);
    ...
    for (const request of requests) expect((await instance.inject({ ...request, headers: { authorization } })).statusCode).toBe(403);
  });
