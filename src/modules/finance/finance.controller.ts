...
export class FinanceController {
  ...
  createQuote = async (request: FastifyRequest<{ Body: CreateQuoteInput }>, reply: FastifyReply): Promise<void> => { await reply.status(201).send(await this.service.createQuote(request.body)); };
  ...
  updateQuoteStatus = async (request: FastifyRequest<{ Params: IdParams; Body: UpdateQuoteStatusInput }>, reply: FastifyReply): Promise<void> => { await reply.send(await this.service.updateQuoteStatus(request.params.id, request.body)); };
  ...
  createPayment = async (request: FastifyRequest<{ Params: QuoteIdParams; Body: CreatePaymentInput }>, reply: FastifyReply): Promise<void> => { await reply.status(201).send(await this.service.createPayment(request.params.quoteId, request.body)); };
  updatePaymentStatus = async (request: FastifyRequest<{ Params: IdParams; Body: UpdatePaymentStatusInput }>, reply: FastifyReply): Promise<void> => { await reply.send(await this.service.updatePaymentStatus(request.params.id, request.body)); };
}
